...
const KEY='sushiClickerV4';
let S=fresh(),buyAmt=1;
let frenzyUntil=0,clickFrenzyUntil=0,inspUntil=0,rainUntil=0,catBoostUntil=0,stormUntil=0,michUntil=0,emerUntil=0,sacUntil=0,rbowUntil=0;
...
streak:0,lastDay:'',dailyReady:true,exps:[],expsDone:0,stocks:{nori:0,tuna:0,sake:0},prices:{nori:100,tuna:500,sake:2500},
research:{},activeCh:'',doneCh:{},rival:null,lucky:0,lastGold:0,bestRun:0,bestCps:0,fishWins:0,
krakenHp:0,krakenMax:0,bossWins:0,set:{particles:true,shake:true,full:false}};}
...
let g=1;if(S.upgrades.soy1)g*=1.5;if(S.upgrades.soy2)g*=2;if(S.upgrades.soy3)g*=2;if(S.upgrades.soy4)g*=1.5;if(S.upgrades.soy5)g*=1.5;
if(Date.now()<inspUntil)g*=1.25;if(Date.now()<michUntil)g*=1.5;if(season().n.includes('Summer'))g*=1.15;
if(Date.now()<sacUntil)g*=2;if(Date.now()<emerUntil)g*=3;if(Date.now()<rbowUntil)g*=5;
return s*g*(1+DEX.filter(d=>S.totalEarned>=d.at).length*0.01)*(1+Math.floor(Object.keys(S.ach).length/3)*0.02)*(1+S.stars*0.05)*ringBonus()*mileBonus()*chalBonus()*resBonus()*(S.activeCh==='slow'?0.5:1)*(S.pUp.p_soul?1.25:1);}
...
if(S.upgrades.click5)g+=rawCps()*0.15;if(S.upgrades.click8)g+=rawCps()*0.02;
